import { useSite } from '../hooks/useSite'

/**
 * Botón flotante de WhatsApp.
 *
 * Siempre visible: antes se ocultaba hasta pasar el 60% del hero, y eso
 * escondía la vía de contacto principal justo donde más se mira.
 *
 * Va en el verde oficial de WhatsApp — el único color del sitio fuera de
 * la paleta del manual — porque se reconoce al instante. El halo que late
 * se detiene solo si el sistema pide menos movimiento (@media reduce).
 */
export default function WhatsAppFab() {
  const { whatsappLink } = useSite()

  return (
    <a
      href={whatsappLink()}
      target="_blank"
      rel="noreferrer"
      aria-label="Agendar cita por WhatsApp"
      className="group fixed bottom-6 right-6 z-40 flex h-16 w-16 items-center justify-center"
    >
      <span
        aria-hidden="true"
        /* pointer-events-none: al escalar a 1.75x el halo se vuelve invisible
           pero seguiría capturando clics fuera del botón. */
        className="pointer-events-none absolute inset-0 animate-halo rounded-full bg-whatsapp/50"
      />
      <span className="relative flex h-16 w-16 items-center justify-center rounded-full bg-whatsapp shadow-[0_12px_32px_-8px_rgba(37,211,102,.7)] ring-1 ring-black/5 transition-all duration-500 ease-smooth group-hover:-translate-y-1 group-hover:bg-whatsapp-oscuro group-hover:shadow-[0_18px_40px_-8px_rgba(37,211,102,.85)]">
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
        className="h-8 w-8 text-white"
        aria-hidden="true"
      >
        <path d="M17.47 14.38c-.3-.15-1.75-.86-2.02-.96-.27-.1-.47-.15-.67.15-.2.3-.77.96-.94 1.16-.17.2-.35.22-.64.07-.3-.15-1.25-.46-2.38-1.47-.88-.78-1.47-1.75-1.65-2.05-.17-.3-.02-.46.13-.6.13-.14.3-.35.45-.52.15-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.08-.15-.67-1.6-.92-2.2-.24-.58-.49-.5-.67-.51h-.57c-.2 0-.52.07-.79.37s-1.04 1.01-1.04 2.47c0 1.46 1.06 2.87 1.21 3.07.15.2 2.1 3.2 5.08 4.49.71.3 1.26.49 1.69.63.71.22 1.36.19 1.87.12.57-.09 1.75-.72 2-1.41.25-.69.25-1.28.17-1.41-.07-.13-.27-.2-.57-.35z" />
        <path d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91c0 1.75.46 3.46 1.32 4.96L2 22l5.25-1.38a9.86 9.86 0 0 0 4.79 1.22h.01c5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.9-7.01A9.82 9.82 0 0 0 12.04 2zm0 18.15h-.01a8.2 8.2 0 0 1-4.18-1.15l-.3-.18-3.11.82.83-3.04-.2-.31a8.2 8.2 0 0 1-1.26-4.38c0-4.54 3.7-8.24 8.24-8.24 2.2 0 4.27.86 5.83 2.42a8.19 8.19 0 0 1 2.41 5.83c0 4.54-3.7 8.23-8.25 8.23z" />
      </svg>
      </span>
    </a>
  )
}
